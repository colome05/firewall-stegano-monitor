...
def generar_pcap():
    print("="*60)
    print(" GENERADOR PCAP: CANAL ENCUBIERTO (CAPA 4 - UDP PUERTOS LIMPIO)")
    print("="*60)
    ...
    pkts = []
    tiempo_base = time.time() # Fijamos el tiempo cero de la captura
    puerto_ruido_fijo = 55000 # Puerto constante para el tráfico inocente

    for pos in todas:
        if pos in mapa:
            idx, byte_val, base_id = mapa[pos]
            random_prefix = random.randint(192, 254)
            sport_esteganografico = (random_prefix << 8) | byte_val

            pkt = IP(src=IP_ESTEGANO, dst=IP_DESTINO, tos=0, ttl=64, id=base_id, flags="DF") / \
                  UDP(sport=sport_esteganografico, dport=443) / Raw(os.urandom(1200))
        else:
            # Paquetes de ruido QUIC legítimos (Puerto estático e ID secuencial)
            pkt = IP(src=ip_normal, dst=IP_DESTINO, tos=0, ttl=64, id=0, flags="DF") / \
                  UDP(sport=puerto_ruido_fijo, dport=443) / Raw(os.urandom(1200))

        # Tiempos de red estabilizados
...
def decodificar(archivo, clave, bases, ip_esperada):
    print("\n" + "="*60)
    print(" DECODIFICADOR FORENSE (CAPA 4 UDP PUERTOS)")
    print("="*60)
