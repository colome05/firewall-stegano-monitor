...
def generar_pcap():
    print("="*60)
    print(" GENERADOR PCAP: CANAL ENCUBIERTO (CAPA 7 - PAYLOAD LIMPIO)")
    print("="*60)
    ...
    for pos in todas:
        if pos in mapa:
            ...
            longitud_payload = 1000 + byte_val

            pkt = IP(src=IP_ESTEGANO, dst=IP_DESTINO, tos=0, ttl=64, id=base_id, flags="DF") / \
                  UDP(sport=puerto_origen_fijo, dport=443) / Raw(os.urandom(longitud_payload))
        else:
            # Tráfico de ruido con tamaño perfectamente estático
            longitud_ruido = 1200
            pkt = IP(src=ip_normal, dst=IP_DESTINO, tos=0, ttl=64, id=0, flags="DF") / \
                  UDP(sport=puerto_origen_fijo, dport=443) / Raw(os.urandom(longitud_ruido))

        # Tiempo absoluto basado en la posición de red
...
def decodificar(archivo, clave, bases, ip_esperada):
    print("\n" + "="*60)
    print(" DECODIFICADOR FORENSE (CAPA 7 PAYLOAD)")
    print("="*60)
    ...
    for pkt in pkts:
        ...
        for i, base in enumerate(bases):
            if ip.id == base:
                longitud_total = len(pkt[Raw])
                byte_val = longitud_total - 1000
                if 0 <= byte_val <= 255:
